...
interface BowlingStore {
  ...
  setBallPositionX: (x: number) => void;
  aimAngle: number;
...
export const useStore = create<BowlingStore>((set, get) => ({
  ...
  ballPositionX: 0,
  setBallPositionX: (x) => set({ ballPositionX: x }),
  aimAngle: 0,
  ...
  advanceRoll: (pinsDownThisRoll) => {
    ...
    if (isTurnOver) {
      if (state.currentFrame < state.totalFrames - 1) {
        ...
        if (state.gameMode === 'single' || state.currentPlayerIndex === state.players.length - 1) {
