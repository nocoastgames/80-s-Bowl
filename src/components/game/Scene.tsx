...
function AimGuide() {
  ...
  useFrame(() => {
    ...
    if (groupRef.current) {
      groupRef.current.position.x = state.ballPositionX;
      groupRef.current.visible = state.playState === 'positioning' || state.playState === 'aiming';
    }
    if (pivotRef.current) {
      pivotRef.current.rotation.y = state.playState === 'positioning' ? 0 : state.aimAngle;
    }
  });
    ...
    if (playState === 'scoring') {
      ...
      if (rollTimer.current > 3) {
        ...
        if (nextState.gameState !== 'results' && !nextState.teacherAdvancePending) {
          ...
          if (isNextFrame || isLastFrameReset) {
          ...
          setTimeout(() => {
            useStore.setState({ ballPositionX: 0 });
            setPlayState('positioning');
